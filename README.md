 ...
 **שכבת הקישור והעברה (Link Layer & Sending):**
 ...
 **אימות מבנה המנה מהלכידה הממשית:**
   * ניתחנו את השדות הטכניים כפי שהם מופיעים ב-Wireshark:
     * **פורט-Source/Destination Port:** זיהינו את הפורטים כפי שהוגדרו ( פורט `12345` ו-`42069`).
     * **מספר סידורי-Sequence Numbers:** ניתן לראות כיצד כל מנה מקבלת מספר סידורי המאפשר את הרכבת המידע בצד המקבל.
     * **דגלים-Flags:** זיהינו שימוש בדגלי **PSH (Push)** ו-**ACK** (ערך הקסדצימלי `0x18`), המורים למערכת ההפעלה להעביר את הנתונים מיד לאפליקציה מבלי להמתין למילוי מלא של המאגר (Buffer).
       
<a id="part1-analysis"></a>
## תיאור והסבר של תעבורה שנלקדה ב-Wireshark :


###  מבנה השכבות (The OSI Model in Practice)
...
###  פורטים בשכבת התעבורה (TCP Ports)
...
השרת (`server.py`) פועל בצורה **רב-תהליכית (Threads)** ומנהל "מילון לקוחות" (Clients Dictionary):
...
### 2. הלקוח (Client-Side)
הלקוח (`main.py`) נבנה בעזרת ספריית `Kivy`:
...
**יש לוודא שיש פייתון בגרסה 3.13! לא יעבוד עם גרסה 3.14**

### ⚙️ דרישות מוקדמות וספריות (Imports)
לפני ההרצה, יש לוודא שמותקנות כל הספריות הבאות (אלו הספריות שנעשה להן Import בקוד):
* **ספריית-Kivy**: עבור ממשק הלקוח.
**לוודא שיש פייתון בגרסה 3.13! לא יעבוד עם גרסה 3.14**
* **ספריית-CustomTkinter**: עבור ממשק השרת.
* **ספריית-python-dotenv**: לניהול משתני סביבה.
* **ספריית-Socket & Threading**: (רק לבדוק שקיימות) ספריות מובנות לניהול התקשורת.
* **ספריית-JSON**: (קיים בפייתון בדיפולט) לעיבוד הפרוטוקול. 

```bash
pip install kivy python-dotenv customtkinter
```

### הגדרת קובץ .env
יש ליצור קובץ בשם `.env` בתיקיית השורש של הפרויקט (`Lord-of-the-Pings/.env`).

**להרצה לוקאלית על אותו מחשב:**
```bash
HOST="127.0.0.1"
```

**משתני סביבה נוספים (אופציונליים):**
```bash
# כתובת השרת - 0.0.0.0 מאפשר חיבורים מכל ממשק רשת
SERVER_HOST="0.0.0.0"

# פורט השרת הראשי לתקשורת TCP (ברירת מחדל: 9000)
SERVER_PORT=9000

# האם לאפשר חיפוש אוטומטי של פורט פנוי אם הפורט הראשי תפוס (ברירת מחדל: true)
SERVER_PORT_AUTO_FALLBACK=true

# פורט ה-UDP לגילוי שרת ברשת (ברירת מחדל: 9001)
DISCOVERY_PORT=9001

# זמן המתנה בשניות בין ניסיונות גילוי שרת (ברירת מחדל: 2)
DISCOVERY_RETRY_INTERVAL=2

# כתובת ה-IP של השרת עבור הלקוח (אם מוגדר, הלקוח לא יבצע גילוי אוטומטי)
HOST="127.0.0.1"
```

### הרצת המערכת
חובה להיות בתיקייה המתאימה לפני כל פקודה (`cd` לתיקיית server או client).

1. **הפעלת השרת:** פתחו טרמינל, עברו לתיקיית השרת והריצו: 
```bash
 cd Part_2/server
 python server.py
```
2. **הפעלת לקוחות:** בטרמינל נפרד, עברו לתיקיית הלקוח והריצו (ניתן לפתוח מספר טרמינלים עבור משתמשים שונים):
```bash
 cd Part_2/client
 python main.py
```
